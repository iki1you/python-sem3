...
def get_format_table(filename, start, end, fields, data_filter):
...
def get_total_table():
    ...
    if ':' not in data_filter:
        return 'Формат ввода некорректен'
    key_filter, data = data_filter.split(':')
    if key_filter not in filter_fields:
        return 'Параметр поиска некорректен'
    fields.append('№')
    return get_format_table(filename, start, end, fields, data_filter).get_string(start=start, end=end, fields=fields)


def filter_table(table, data_filter):
    return table
...
filter_fields = {
    'Название': 'simple_compare',
    'Описание': 'simple_compare',
    'Компания': 'simple_compare',
    'Навыки': 'skills_compare',
    'Опыт работы': 'simple_compare',
    'Премиум-вакансия': 'premium_compare',
    'Оклад': 'salary_compare',
    'Название региона': 'simple_compare',
    'Дата публикации вакансии': 'date_compare',
    'Идентификатор валюты оклада': 'currency_compare'
}

print(get_total_table())
