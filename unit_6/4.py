...
def csv_parser(file_name):
    ...
    vacancies = csv_filer(reader, naming)
    return vacancies
...
def get_table(data_vacancies, filter_arg, sort_key, reverse_arg):
    ...
    data_vacancies = sort_table(data_vacancies, sort_key, reverse_arg)
...
def get_format_table(filename, start, end, fields, filter_arg, sort_key, reverse_arg):
    ...
    if reader == 'Пустой файл':
        return 'Пустой файл'
    table = get_table(csv_filer(*reader), filter_arg, sort_key, reverse_arg)
    if table == 'Нет данных':
        return 'Нет данных'
    if end is None:
        end = len(table.rows)
