...
def parse_data(data):
    rows = list(filter(lambda x: len(x) == len(data[0]), filter(lambda x: all([len(e) > 0 for e in x]), data[1:])))
...
def get_salary_list(data, value):
    ...
    for (i, vacancy) in enumerate(data[:10]):
...
with open(filename, encoding='utf-8-sig') as file:
    ...
    parsed_data = filter_currency(parse_data(raw))
